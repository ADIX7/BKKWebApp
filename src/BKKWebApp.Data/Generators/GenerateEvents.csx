...
var rootFolderFullName = new DirectoryInfo(Path.Combine(folderName, "../../")).FullName;


foreach (var jsonFile in dataDir.GetFiles("*.json"))
{
    var jsonRelativePath = jsonFile.FullName.Replace(rootFolderFullName, "");

    Console.WriteLine(jsonRelativePath);
    ...
    foreach (var child in jObject.Root)
    {
        ...
        Output.WriteLine($@"//THIS FILE IS AUTOGENERATED BY {jsonRelativePath}
using System;
...
{{
    ...
    {{
        public const string eventName = ""{nameProvider.GetPastName().FirstLetterToLowerCase()}"";
");
        ...
        Output.WriteLine($@"
        internal {nameProvider.GetPastName()}Event(string source) : base(eventName)
        {{
            var sourceObj = JObject.Parse(source);
            throw new NotImplementedException();
        }}

        public {nameProvider.GetPastName()}Event(Guid aggregateId, int version{(parameters.Count == 0 ? "" : ", " + string.Join(", ", parameters))}) : base(aggregateId, version, eventName)
        {{");
...
{{
    ...
    {{");
        ...
        foreach (var prop in classData["properties"])
        {
            ...
            Output.WriteLine($"\t\tpublic {type} {propName.FirstLetterToUpperCase()} {{ get; }}");
            parameters.Add(type + " " + propName.FirstLetterToLowerCase());
        }
